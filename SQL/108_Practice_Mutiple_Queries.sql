...
As our transaction data is at product_id level, to get this data, we will need to aggregate twice, once to the transaction_id level, and then again to find out the maximum amount spent per transaction, per customer.

Use either temporary tables or CTE to get this information.

Your query should return one row per customer, showing their maximum transaction spend. To make it easy for the stakeholder, please make sure the output is sorted by customer_id.
...
create temp table prac as (

select
  customer_id,
  transaction_id,
  sum(sales_cost) as total_spent
...
select 
  customer_id,
  max(total_spent) as max_trans_sales
